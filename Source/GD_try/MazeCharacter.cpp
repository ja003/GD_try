...
void AMazeCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	InputComponent->BindAxis("RotateRight", this, &AMazeCharacter::RotateFloorRight);

}

void AMazeCharacter::RotateFloorRight(float Value) {

	//AMazeGameModeBase* mazeGameMode = (AMazeGameModeBase*)GetGameMode();
	AMazeGameModeBase* mazeGameMode = Cast<AMazeGameModeBase>(GetWorld()->GetAuthGameMode());

	mazeGameMode->Floor->RotateRight(Value);
}
